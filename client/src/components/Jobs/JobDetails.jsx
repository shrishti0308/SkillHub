    ...
    <div className="min-h-screen  text-white p-8 flex flex-col items-center">
      <div className="max-w-3xl w-full bg-gray-800 p-6 rounded-lg shadow-lg">
        ...
        <div className="flex justify-between mb-4">
          <div>
            <span className="font-semibold">Budget:</span> ₹{job.budget.min} - ₹
            {job.budget.max}
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap mt-2">
            {job.skillsRequired.map((skill, index) => (
              <span
                key={index}
                className="bg-gray-500 text-white rounded-full px-3 py-1 text-sm mr-2 mb-2"
              >
                {skill}
              </span>
            ))}
        ...
        <div className="mb-4">
          ...
          <div className="flex flex-wrap mt-2">
            {job.categories.map((category, index) => (
              <span
                key={index}
                className="bg-gray-600 text-white rounded-full px-3 py-1 text-sm mr-2 mb-2"
              >
                {category}
              </span>
            ))}
        ...
        <div className="flex items-center mb-4 border-t border-gray-700 pt-4">
          {job.employer.info.profilePic ? (
            ...
            <FontAwesomeIcon
              icon={faUserCircle}
              className="w-12 h-12 text-gray-500 mr-3"
            />
          )}
          <a
            ...
            <li className="bg-gray-700 p-4 rounded-md flex items-center justify-between">
              <div className="flex items-center">
                {acceptedBid.freelancer.info.profilePic ? (
                  ...
                  <FontAwesomeIcon
                    icon={faUserCircle}
                    className="w-10 h-10 text-gray-500 mr-2"
                  />
                )}
                <a
              ...
              <div className="flex items-center">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold bg-gray-500 text-white`}
                >
                  {acceptedBid.status.charAt(0).toUpperCase() +
                    acceptedBid.status.slice(1)}
                </span>
              </div>
            </li>
          </div>
        ...
        <h2 className="text-2xl font-bold mt-6 border-t border-gray-700 pt-4">
          Bids for this Job
        </h2>
        {bids.length > 0 ? (
