    ...
    <div className="min-h-screen flex flex-col md:flex-row">
      ...
      <div className="bg-gradient-to-br from-blue-500 to-blue-700 w-full md:w-1/3 py-20 flex flex-col items-center justify-center">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-gray-200 text-center px-2 mx-2 md:mx-4 border-x-4">
          ...
          <div className="space-y-4 mt-4">
            {reviews.length > 0 ? (
              reviews.map((review, index) => (
                <div
                  key={index}
                  className="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-lg w-fit"
                >
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mt-2 inline-block mx-auto">
                    {review.comment}
                  </p>
                </div>
              ))
...
const UserDetailsSection = ({ title, items }) => (
  <section className="w-full">
    <h2 className="mt-8 text-3xl font-bold text-gray-800 dark:text-gray-200 mb-3">
      {title}
    </h2>
    {items.length > 0 ? (
...
const PreviousWorksSection = ({ works }) => (
  <section className="w-full">
    <h2 className="mt-8 text-3xl font-bold mb-3 text-gray-800 dark:text-gray-200">
      Previous Works
    </h2>
    {works.length > 0 ? (
      <ul className="mt-2 space-y-4 w-full">
        {works.map((work, index) => (
          <li
            key={index}
            className="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-lg transition duration-300 hover:shadow-xl"
          >
            <a
              href={work.link}
