...
const AdminDashboard = () => {
    ...
    const { admins = [], loading, error, success, currentAdmin } = useSelector((state) => state.admin);
    ...
    const handleDeleteClick = (admin) => {
        ...
        if (admin.role === 'superuser') {
            const superuserCount = admins.filter(a => a.role === 'superuser').length;
            if (superuserCount <= 1) {
                dispatch({ 
                    type: 'admin/setError', 
        ...
        <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            ...
            <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                    ...
                    {(currentAdmin?.role === 'superuser' || currentAdmin?.permissions.includes('manageAdmins')) && (
                        <Button
                            variant="contained"
                            color="primary"
                                ...
                                <TableRow key={admin._id}>
                                    ...
                                    <TableCell align="right">
                                        ...
                                        <IconButton
                                            ...
                                            color="primary"
                                           disabled={(!currentAdmin?.permissions.includes('manageAdmins') || admin.role === 'superuser')}
                                        >
                                            <EditIcon />
                                        </IconButton>
                                    </TableCell>
                ...
                {/* Permissions Dialog */}
                <Dialog open={permissionsDialogOpen} onClose={handleClosePermissionsDialog}>
                    <DialogTitle>
