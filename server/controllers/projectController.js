...
exports.getRecentProjects = async (req, res) => {
  try {
    ...
    if (!recentProjects.length) {
      return res.status(404).json({ message: 'No recent projects found for this freelancer' });
    }

    res.status(200).json({ recentProjects });
  } catch (error) {
    res.status(500).json({ message: 'Error retrieving recent projects', error: error.message });
  }
};
