...
exports.addReview = async (req, res) => {
    ...
    try {
        ...
        if (existingReview) return res.status(400).json({ success: false, message: 'You have already reviewed this user' });
...
exports.updateReview = async (req, res) => {
    ...
    try {
        ...
        if (!review) return res.status(404).json({ success: false, message: 'Review not found or you are not the reviewer' });
...
exports.getAllReviewsByUser = async (req, res) => {
    ...
    try {
        const reviews = await Review.find({ reviewer: userId }).populate('reviewedUser', 'name');
        res.status(200).json({ success: true, reviews });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error fetching reviews', error });
    }
